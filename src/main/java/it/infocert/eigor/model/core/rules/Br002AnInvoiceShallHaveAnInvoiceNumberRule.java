...
public class Br002AnInvoiceShallHaveAnInvoiceNumberRule implements Rule {
    ...
    public RuleOutcome isCompliant(CoreInvoice coreInvoice) {
        if(coreInvoice.getBt01InvoiceNumbers().size() == 1){
            return RuleOutcome.newOutcome(
                    RuleOutcome.Outcome.SUCCESS,
                    "An invoice shall have an invoice number, it has: %s.", coreInvoice.getBt01InvoiceNumbers().get(0)
            );
        }else{
            return RuleOutcome.newOutcome(
                    RuleOutcome.Outcome.FAILED,
                    "An invoice shall have an invoice number, but it has %d.", coreInvoice.getBt01InvoiceNumbers().size()
            );
        }
    }
