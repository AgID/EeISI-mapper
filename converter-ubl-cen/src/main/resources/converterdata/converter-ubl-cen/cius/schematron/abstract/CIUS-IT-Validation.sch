...
Buyer identifier identification scheme identifier) -BT46-1=IT:CF then BT-46 minimum lenght 11 and maximum lenght shall be 16 
		</assert>
		<assert test="$CIUS-VD-100-2" id="CIUS-VD-100-2" flag="fatal"> [CIUS-VD-100-2] BT-46, BT-46-1 (Buyer identifier
Buyer identifier identification scheme identifier) -If BT-46-1=IT:EORI then BT-46 minimum lenght 13 and maximum lenght shall be 17 
		</assert>
		<assert test="$CIUS-VD-100-3" id="CIUS-VD-100-3" flag="fatal"> [CIUS-VD-100-3] BT-46, BT-46-1 (Buyer identifier
Buyer identifier identification scheme identifier) -If BT-46-1=IT:VAT then BT-46 maximum length 30 (the first two chars indicates country code). 
		</assert>
	</rule>
...
Seller identifier identification scheme identifier) -BT29-1=IT:CF then BT-29 minimum lenght 11 and maximum lenght shall be 16. 
		</assert>
		<assert test="$CIUS-VD-101-2" id="CIUS-VD-101-2" flag="fatal"> [CIUS-VD-101-2] BT-29, BT-29-1 (Seller identifier
Seller identifier identification scheme identifier) -If BT-29-1=IT:EORI then BT-29 minimum lenght 13 and maximum lenght shall be 17 . 
		</assert>
		<assert test="$CIUS-VD-101-3" id="CIUS-VD-101-3" flag="fatal"> [CIUS-VD-101-3] BT-29, BT-29-1 (Seller identifier
Seller identifier identification scheme identifier) -If BT-29-1=IT:VAT then BT-29 maximum length 30 (the first two chars indicates country code). 
		</assert>
	</rule>
	<rule context="$Accounting_Supplier_PartyLegalEntity">
		<assert test="$CIUS-VD-102-1" id="CIUS-VD-102-1" flag="fatal"> [CIUS-VD-102-1] BT-30, BT-30-1 (Seller legal registration identifier
Seller legal registration identifier identification scheme identifier) -If BT-30-1=IT:REA then BT-30 minimum lenght 3 and maximum lenght shall be 22 (first two chars indicate the italian province code). 
		</assert>
		<assert test="$CIUS-VD-102-2" id="CIUS-VD-102-2" flag="fatal"> [CIUS-VD-102-2] BT-30, BT-30-1 (Seller legal registration identifier
Seller legal registration identifier identification scheme identifier) -If BT-30-1=IT:ALBO then BT-30 maximum length 60 . 
		</assert>
	</rule>
...
BT-49-1 (Buyer electronic address
Buyer electronic address identification scheme identifier) shall contain a legal mail address (PEC) or IndicePA/CodiceDestinatario (see the Italian business rules). BT-49-1=IT:PEC or IT:IPA or IT:CODDEST 
		</assert>
...
Buyer identifier identification scheme identifier) -If BT-48 is empty then one of the buyer identifiers (0..n) should be the FiscalCode in BT-46. BT-46-1 shall contain the scheme. 
		...
		<assert test="$CIUS-VD-97-1" id="CIUS-VD-97-1" flag="fatal"> [CIUS-VD-97-1-1] BT-49, BT-49-1 (Buyer electronic address
Buyer electronic address identification scheme identifier) -If BT-49-1=IT:PEC schema then BT-49 minimum length shall be 7 maximum length shall be 256 
		</assert>
		<assert test="$CIUS-VD-97-2" id="CIUS-VD-97-2" flag="fatal"> [CIUS-VD-97-1-2] BT-49, BT-49-1 (Buyer electronic address
Buyer electronic address identification scheme identifier) =IT:IPA schema then BT-49 maximum length shall be 6 chars 
		</assert>
		<assert test="$CIUS-VD-97-3" id="CIUS-VD-97-3" flag="fatal"> [CIUS-VD-97-1-3] BT-49, BT-49-1 (Buyer electronic address
Buyer electronic address identification scheme identifier)=IT:CODDEST schema then BT-49 maximum length shall be 7 chars. 
		</assert>
	</rule>
...
Buyer address line 3) -The sum of BTs maximum length shall be 60 chars (including separator). 
		</assert>
...
Seller address line 3) -The sum of BTs maximum length shall be 60 chars (including separator). 
		</assert>
...
Deliver to address line 3) -The sum of BTs maximum length shall be 60 chars (including separator). 
		</assert>
