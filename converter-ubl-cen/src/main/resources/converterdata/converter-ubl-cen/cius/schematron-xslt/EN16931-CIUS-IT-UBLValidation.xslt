...
<!--PATTERN CIUS-IT-->
	...
	<!--RULE -->
<xsl:template match="cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification" mode="M7" priority="1090">
...
<xsl:choose>
      ...
      <xsl:otherwise>
        <svrl:failed-assert test="not (cbc:ID/@schemeID = ' IT:CF') or ( (string-length(cbc:ID) >= 11) and (string-length(cbc:ID) &lt;=16))">
          ...
          <svrl:text> [CIUS-VD-100] BT-46, BT-46-1 (Buyer identifier
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="not (cbc:ID/@schemeID = ' IT:VAT')  or ( (string-length(cbc:ID) &lt;= 30) )" />
      <xsl:otherwise>
        <svrl:failed-assert test="not (cbc:ID/@schemeID = ' IT:VAT') or ( (string-length(cbc:ID) &lt;= 30) )">
          <xsl:attribute name="id">CIUS-VD-100-3</xsl:attribute>
          <xsl:attribute name="flag">fatal</xsl:attribute>
          <xsl:attribute name="location">
            <xsl:apply-templates mode="schematron-select-full-path" select="." />
          </xsl:attribute>
          <svrl:text> [-2]  () -If BT-48 is empty then one of the buyer identifiers (0..n) should be the FiscalCode in BT-46. BT-46-1 shall contain the scheme.
        </svrl:text>
        </svrl:failed-assert>
      </xsl:otherwise>
    </xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:AccountingSupplierParty/cac:Party/cac:PartyTaxScheme" mode="M7" priority="1089">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:PaymentMeans/cac:PayeeFinancialAccount" mode="M7" priority="1088">
...
<xsl:choose>
	...
	<!--RULE -->
<xsl:template match="cac:AccountingCustomerParty/cac:Party" mode="M7" priority="1087">
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="not(exists(cbc:CityName))" />
      <xsl:otherwise>
        <svrl:failed-assert test="not(exists(cbc:CityName))">
...
<xsl:template match="cac:PaymentMeans" mode="M7" priority="1039">
    <svrl:fired-rule context="cac:PaymentMeans" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="string-length(cbc:PaymentID) &lt;= 6" />
      <xsl:otherwise>
        <svrl:failed-assert test="string-length(cbc:PaymentID) &lt;= 6">
...
<xsl:template match="cac:PaymentMeans/cac:PayeeFinancialAccount" mode="M7" priority="1038">
    <svrl:fired-rule context="cac:PaymentMeans/cac:PayeeFinancialAccount" />
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="string-length(cbc:ID) >= 3400 and string-length(cbc:ID) >= 15" />
      <xsl:otherwise>
        <svrl:failed-assert test="string-length(cbc:ID) >= 3400 and string-length(cbc:ID) >= 15">
...
<xsl:choose>
      ...
      <xsl:otherwise>
		...
		<!--ASSERT -->
<xsl:choose>
      <xsl:when test="string-length(@listID) >= 35000" />
      <xsl:otherwise>
        <svrl:failed-assert test="string-length(@listID) >= 35000">
