...
public class Ubl2Cen extends AbstractToCenConverter {
    ...
    public void configure() throws ConfigurationException {
        ...
        {
            try {
                xsdValidator = new ClasspathXSDValidator("/converterdata/converter-commons/ubl/xsdstatic/UBL-Invoice-2.1.xsd", ErrorCode.Location.UBL_IN);
            } catch (Exception e) {
        ...
        try {
            ...
            ublValidator = new SchematronValidator(ublSchemaFile, true, schematronAutoUpdate, ErrorCode.Location.UBL_IN);
        } catch (Exception e) {
    ...
    private void applyCustomMapping(BG0000Invoice invoice, Document document, List<IConversionIssue> errors) {
        ...
        for (CustomMapping<Document> customMapping : customMappings) {
            customMapping.map(invoice, document, errors, ErrorCode.Location.UBL_IN, null);
        }
    }
