...
-->
    ...
    <rule context="BT-1">
      <!--  
        ...
        (ENG): BT maximum length shall be 20 chars with at least one digit
        
        BT-1: /Invoice/cbc:ID
        /CreditNote/cbc:ID
        
        Restriction: BT maximum length shall be 20 chars with at least a digit.
      -->   
    ...
    <rule context="BT-30">
      ...
      <!--assert
        ...
        id="BR-IT-110" 
        flag="fatal"> [BR-IT-110] BT-30 (Seller legal registration identifier) - BT-30 minimum length 10 and maximum length shall be 29 starting with "IT:REA:" and the following two chars indicate the italian province code). 
        </assert--> 
    ...
    <rule context="BT-48">
      <!--  
        ...
        Restriction:  BT maximum length shall be 30 chars
      -->       
    ...
    <rule context="BT-99">
      ...
      <assert test=  "matches(normalize-space(.),'^[\-]?[0-9]{1,11}([\.][0-9]{1,8})?$')" 
        id="BR-IT-290-1" 
        flag="fatal">  [BR-IT-290-1]  BT-99 (Document level allowance amount - Document level charge amount) - BT maximum length shall be 21. 
      </assert>
    </rule>
    ...
    <rule context="BT-95 | BT-102 | BT-118 | BT-151" >
      <!--  
        ...
        EeISI Rule 
        (ITA): I valori accettati sono esclusivamente AE E S B G  K 
      ...
      <assert test="( ( not(contains(normalize-space(.),' ')) and contains( ' AE E S B G K ',concat(' ',normalize-space(.),' ') ) ) )"
        id="BR-IT-350-1" 
        flag="fatal"> [BR-IT-350] VAT category codes - only values E (Exempt from VAT), K	(Intra-community supply), AE (Reverse charge), G (Export outside the EU) ,S (Standard rated), B (Split payment) shall be allowed . 
      </assert>
    </rule>
    <!--	
    <rule context="BT-102" >
