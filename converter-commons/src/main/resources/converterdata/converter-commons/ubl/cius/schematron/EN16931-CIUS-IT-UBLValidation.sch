...
2. Added NEW rules  
  ...
  6. BR-IT-490 commented
   ...
   Change Log 2019-03-14
   1. BR-IT-360 added DocumentTypecode 50 
...
-->
   ...
   -->
      <assert test=  "$customerCountry!='IT' or (exists(cbc:EndpointID) and 
        ...
        or cbc:EndpointID[normalize-space(@schemeID) = '0201'] ))"
        id="BR-IT-190" 
    ...
    eEISI CIUS Referemce: BR-IT-200-1, BR-IT-200-2, BR-IT-200-3
     ...
     (ITA): Se il valore dell’elemento BT-55 Buyer country code è ”IT”, 
            ...
            Altrimenti, se l'elemento BT-49-1 Buyer electronic address identification scheme identifier contiene il valore "IT:IPA" (0201),
    ...
    -->      
      ...
      <assert test=  "$customerCountry!='IT' or  not(cbc:EndpointID[normalize-space(@schemeID) = '0201'])
  ...
  EeISI Rule
   ...
   -->      
      <assert test="$customerCountry != 'IT' or not(cbc:ID[starts-with(normalize-space(.), 'IT:CF')]) or matches( normalize-space(cbc:ID[starts-with(normalize-space(.), 'IT:CF')]), '(^IT:CF:[A-Z0-9]{11,16}$)')"
        id="BR-IT-160-1" 
        flag="fatal"> [BR-IT-160] BT-46 (Buyer identifier) - BT-46 minimum length 17 and maximum length shall be 22 starting with "IT:CF: ". 
      </assert>
    </rule>
  ...
  -->
    ...
    <rule context="cac:AdditionalDocumentReference[(normalize-space(cbc:DocumentTypeCode)!='130' and normalize-space(cbc:DocumentTypeCode)!='50') or not(exists(cbc:DocumentTypeCode))]">        
    <!--  
    ID: BT-125 - BT-124
    Cardinality:  0..1	- 0..1
    Level: 2 - 2
   ...
   Nelle ipotesi precedenti si univa la data al numero DDT e la regola era del tipo
    ...
    Business Term: 	Item attribute name
    Description:  The name of the attribute or property of the item.
    eEISI CIUS Reference: BR-IT-480
